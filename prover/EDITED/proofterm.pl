...
normal0(mu(covar(Alpha),S),mu(covar(Alpha),S1)) :- 
	normal(S,S1).
normal(Term,_) :- sub_term(mu(covar(_),_),Term),!,fail.
normal(covar(Alpha),lambda(var(X),appl(covar(Alpha),var(X)))) :- !.
normal(Term,Term).

pol(pos,Form) :- Form=..[C|_],member(C,[lit,otimes,oslash,obslash,rdneg,ldneg]).
...
%	concat_atom(['/Users/mjm/Projects/LG/Jeroen/LGprover2/LGprover','lex.txt','"',Input,'"','"',Goal,'"','1> /dev/null'],
%		' ',Derivations),
%	shell(Derivations),
	shell(Command),
	yield(In,String),
...
show_terms(String) :-
	...
	words(Product,Term0,String,[]),
	numbervars(Term0,0,_),
%	normal0(Term0,Term),
	write('\\['),
	write_sem(Term0,1),
	write('\\]'),
