...
\autoref{fig:syntactic-calculus} is not especially suited to proof
...
\autoref{fig:display-calculus}.\footnote{%
  The calculus in \autoref{fig:display-calculus} was first formulated
...
rules to navigate them.\footnote{%
  ...
  $\struct{\ }$-connective, this is perfectly unambiguous.
}
These two work together to guarantee the \textbf{display
...
the display postulate (DP):\footnote{%
  We have attached a formal proof of the display property for full
  NLQ---an extension of display NL as presented in this section. See
...
One difference between our display calculus and \citepos{lambek1961}
sequent calculus is that we use the product-free version of NL. The
reason for this is that we do not have solid linguistic evidence to
include the product type. However, if it turns out to be useful, it is
very easily added:
\begin{center}
  \begin{pfbox}
    \AXC{$\struct{A}\prod\struct{B}\fCenter{X}$}
    \RightLabel{L$\prod$}
    \UIC{$\struct{A\prod{B}}\fCenter{X}$}
  \end{pfbox}
  \begin{pfbox}
    \AXC{$X\fCenter\struct{A}$}
    \AXC{$Y\fCenter\struct{B}$}
    \RightLabel{R$\prod$}
    \BIC{$X\prod{Y}\fCenter\struct{A\prod{B}}$}
  \end{pfbox}
\end{center}
...
\citet{bastenhof2011} and \citet{moortgat2009,moortgat2012} for
classical non-associative Lambek calculus (CNL) and the Lambek-Grishin
calculus (LG), respectively. Both of these calculi are extensions of
NL, so applying the techniques developed in their work to display NL
is trivial.
