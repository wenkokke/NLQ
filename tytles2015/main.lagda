...
of information: a function |⌈_⌉ᵁ|, which translates the atomic
...
\begin{code}
  module translation (⌈_⌉ᵁ : Univ → Set) (R : Set) where
\end{code}
...
\end{code}
\end{hide}
We will instantiate the type for atomic formulas to |Univ|, as defined
below:
\begin{code}
  data Univ : Set where N NP S : Univ
\end{code}
Last, we need to define a function which maps the values of |Univ| to
...
\begin{code}
  ...
  ⌈ S   ⌉ᵁ = Bool
\end{code}
Now that we have |Univ| and |⌈_⌉ᵁ|, we can open up the modules defined
as above, instantiating the return type |R| with the type of booleans.
\begin{code}
  open logic              Univ
  open logic.translation  Univ ⌈_⌉ᵁ Bool
\end{code}
\begin{hide}
\begin{code}
  n np s : Type
...
\begin{hide}
\begin{code}
  sent₀ sent₁ sent₂ sent₃ sent₄ sent₅ sent₆ : Bool
\end{code}
\end{hide}
\setboolean{partial}{true}%
\begin{code}
  sent₀  = ⌈ sᴇɴᴛ₀ ⌉ᴿ (someone , loves , everyone) id
         --> forAll (λ y → ᴘᴇʀsᴏɴ y ⊃ exists (λ x → ᴘᴇʀsᴏɴ x ∧ ʟᴏᴠᴇs x y)))
  sent₁  = ⌈ sᴇɴᴛ₁ ⌉ᴿ (someone , loves , everyone) id
         --> exists (λ x → ᴘᴇʀsᴏɴ x ∧ forAll (λ y → ᴘᴇʀsᴏɴ y ⊃ ʟᴏᴠᴇs x y))
\end{code}
\setboolean{partial}{false}%
\begin{hide}
\begin{code}
...
\begin{hide}
\begin{code}
  -- This last section of code checks the claims made about sent₀
  -- through sent₆'s reduction behaviours above, by comparing the
  -- actual Agda terms to the expected terms.
  -- This is not yet as robust as I'd like it, since there's nothing
  -- stopping me from accidentally copying over the wrong terms.
  -- However, it is still some additional verification, and has
  -- actually helped remove errors from this paper.
  -- In the future, I'll likely be able to improve on this by defining
  -- _↦_ as a macro which quotes both it's arguments and compares them.
  ...
  SurfaceScope : Term
  SurfaceScope =
    quoteTerm (id {A = Bool} (exists (λ x → ᴘᴇʀsᴏɴ x ∧ forAll (λ y → ᴘᴇʀsᴏɴ y ⊃ ʟᴏᴠᴇs x y))))

  InvertedScope : Term
  InvertedScope =
    quoteTerm (id {A = Bool} (forAll (λ y → ᴘᴇʀsᴏɴ y ⊃ exists (λ x → ᴘᴇʀsᴏɴ x ∧ ʟᴏᴠᴇs x y))))

  test₀ : Assert (quoteTerm sent₀ is InvertedScope)
  test₀ = _
  test₁ : Assert (quoteTerm sent₁ is SurfaceScope)
  test₁ = _
  test₂ : Assert (quoteTerm sent₂ is InvertedScope)
  test₂ = _
  test₃ : Assert (quoteTerm sent₃ is SurfaceScope)
