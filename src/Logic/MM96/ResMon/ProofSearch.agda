...
open import Logic.MM96.ResMon.Sequent Atom as J
...
{-# TERMINATING #-}
search : {Mon : Set ℓ → Set ℓ} (monadPlus : RawMonadPlus Mon) (J : Sequent) → Mon (MM96 J)
search {Mon} monadPlus = search′ []
  where
  open RawMonadPlus monadPlus using (∅; _∣_; return; _>>=_)

  search′ : (seen : List Sequent) (J : Sequent) → Mon (MM96 J)
...
findMaybe : (J : Sequent) → Maybe (MM96 J)
findMaybe = search Data.Maybe.monadPlus

find : (J : Sequent) → From-just (MM96 J) (findMaybe J)
find J = from-just (findMaybe J)

findAll : (J : Sequent) → List (MM96 J)
