...
module Logic.MM96.ResMon.Origin.DiaR {ℓ} (Atom : Set ℓ) where
  ...
  data Origin {B} ( J : Contextᴶ - ) (f : MM96 J [ ⟨r⟩ B ]ᴶ) : Set ℓ where
       origin : ∀ {A}
                → (h : MM96 A ⊢ B)
                → (f′ : ∀ {G} → MM96 ⟨r⟩ A ⊢ G → MM96 J [ G ]ᴶ)
                → (pr : f ≡ f′ (m⟨r⟩ h))
                → Origin J f

  mutual
    view : ∀ {B} ( J : Contextᴶ - ) (f : MM96 J [ ⟨r⟩ B ]ᴶ) → Origin J f
    view (._ ⊢> [])       (m⟨r⟩  f)   = origin f id refl
