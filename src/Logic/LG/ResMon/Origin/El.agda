...
module Logic.LG.ResMon.Origin.El {ℓ} (Atom : Set ℓ) where
  ...
  data Origin {B} ( J : Contextʲ + ) (f : LG J [ el B ]ʲ) : Set ℓ where
       origin : (f′ : ∀ {G} → LG G ⊢ el B → LG J [ G ]ʲ)
              → (pr : f ≡ f′ ax)
              → Origin J f


  mutual
    view : ∀ {B} ( J : Contextʲ + ) (f : LG J [ el B ]ʲ) → Origin J f
