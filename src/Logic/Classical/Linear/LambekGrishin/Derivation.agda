...
module Logic.Classical.Linear.LambekGrishin.Derivation {ℓ} (Univ : Set ℓ) where
...
open import Logic.Classical.Linear.LambekGrishin.Base      Univ
...
data LG_⋯_ : (I J : Judgement) → Set ℓ where
  ...
  -- structure
  assᴸ-⊗ : ∀ {J A B C D} → LG J ⋯ A ⊗ (B ⊗ C) ⊢ D → LG J ⋯ (A ⊗ B) ⊗ C ⊢ D
  assᴿ-⊗ : ∀ {J A B C D} → LG J ⋯ (A ⊗ B) ⊗ C ⊢ D → LG J ⋯ A ⊗ (B ⊗ C) ⊢ D
  comm-⊗ : ∀ {J A B C}   → LG J ⋯ A ⊗ B ⊢ C       → LG J ⋯ B ⊗ A ⊢ C

  assᴸ-⊕ : ∀ {J A B C D} → LG J ⋯ A ⊢ B ⊕ (C ⊕ D) → LG J ⋯ A ⊢ (B ⊕ C) ⊕ D
  assᴿ-⊕ : ∀ {J A B C D} → LG J ⋯ A ⊢ (B ⊕ C) ⊕ D → LG J ⋯ A ⊢ B ⊕ (C ⊕ D)
  comm-⊕ : ∀ {J A B C}   → LG J ⋯ A ⊢ B ⊕ C       → LG J ⋯ A ⊢ C ⊕ B
...
module Simple where
  ...
  <>-def f (grish₃ g)     x rewrite <>-def f g  x = refl
  <>-def f (grish₄ g)     x rewrite <>-def f g  x = refl
  <>-def f (assᴸ-⊗ g)     x rewrite <>-def f g  x = refl
  <>-def f (assᴿ-⊗ g)     x rewrite <>-def f g  x = refl
  <>-def f (comm-⊗ g)     x rewrite <>-def f g  x = refl
  <>-def f (assᴸ-⊕ g)     x rewrite <>-def f g  x = refl
  <>-def f (assᴿ-⊕ g)     x rewrite <>-def f g  x = refl
  <>-def f (comm-⊕ g)     x rewrite <>-def f g  x = refl
  ...
  <>-assoc h g (grish₃ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (grish₄ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (assᴸ-⊗ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (assᴿ-⊗ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (comm-⊗ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (assᴸ-⊕ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (assᴿ-⊕ f)     rewrite <>-assoc h g f  = refl
  <>-assoc h g (comm-⊕ f)     rewrite <>-assoc h g f  = refl
  ...
  <>-identityʳ (grish₃ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (grish₄ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (assᴸ-⊗ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (assᴿ-⊗ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (comm-⊗ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (assᴸ-⊕ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (assᴿ-⊕ f)     rewrite <>-identityʳ f  = refl
  <>-identityʳ (comm-⊕ f)     rewrite <>-identityʳ f  = refl
