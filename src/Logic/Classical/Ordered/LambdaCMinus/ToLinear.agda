open import Algebra                               using (module Monoid)
open import Function                              using (_$_)
open import Data.List                             using (List; map; _++_) renaming ([] to ∅; _∷_ to _,_)
open import Data.Product                          using (_,_; proj₂)
open import Relation.Binary.PropositionalEquality using (sym; subst)


module Logic.Classical.Ordered.LambdaCMinus.ToLinear {ℓ} (Univ : Set ℓ) where


open import Logic.Type Univ
open import Logic.Index renaming (lookup to _‼_)
open import Logic.Structure.Conjunction Univ renaming (_[_] to _⟨_⟩)
open import Logic.Classical.Judgement Conjunction Type (List Type) as OJ
open import Logic.Classical.Judgement (List Type) Type (List Type) as LJ
open import Logic.Classical.Ordered.LambdaCMinus.Base Univ as O
open import Logic.Classical.Linear.LambdaCMinus.Base Univ as L


private
  ⟦_⟧ᵀ : Type → Type
  ⟦_⟧ᵀ (el  A) = el A
  ⟦_⟧ᵀ (A ⇒ B) = ⟦ A ⟧ᵀ ⇒ ⟦ B ⟧ᵀ 
  ⟦_⟧ᵀ (B ⇐ A) = ⟦ A ⟧ᵀ ⇒ ⟦ B ⟧ᵀ 
  ⟦_⟧ᵀ (B ⇚ A) = ⟦ B ⟧ᵀ ⇚ ⟦ A ⟧ᵀ   
  ⟦_⟧ᵀ (A ⇛ B) = ⟦ B ⟧ᵀ ⇚ ⟦ A ⟧ᵀ   
  ⟦_⟧ᵀ (A ⊗ B) = ⟦ A ⟧ᵀ ⊗ ⟦ B ⟧ᵀ 
  ⟦_⟧ᵀ (A ⊕ B) = ⟦ A ⟧ᵀ ⊕ ⟦ B ⟧ᵀ 

  ⟦_⟧⁺ : Conjunction → List Type
  ⟦_⟧⁺ (· A ·) = ⟦ A ⟧ᵀ , ∅
  ⟦_⟧⁺ (Γ ⊗ Δ) = ⟦ Γ ⟧⁺ ++ ⟦ Δ ⟧⁺

  ⟦_⟧⁻ : List Type → List Type
  ⟦_⟧⁻ = map ⟦_⟧ᵀ

  ⟦_⟧ : OJ.Judgement → LJ.Judgement
  ⟦_⟧ (Γ OJ.⊢      Δ) = ⟦ Γ ⟧⁺ LJ.⊢          ⟦ Δ ⟧⁻
  ⟦_⟧ (Γ OJ.⊢[ A ] Δ) = ⟦ Γ ⟧⁺ LJ.⊢[ ⟦ A ⟧ᵀ ] ⟦ Δ ⟧⁻


  [_] : ∀ {J} → O.λC⁻ J → L.λC⁻ ⟦ J ⟧
  [ O.ax                ] = L.ax
  [ O.⇒ᵢ            f   ] = L.⇒ᵢ ([ f ])
  [ O.⇒ₑ  {Γ₁ = Γ₁} f g ] = L.sᴸ (⟦_⟧⁺ Γ₁) (L.⇒ₑ ([ g ]) ([ f ]))
  [ O.⇐ᵢ  {A = A}   f   ] = L.⇒ᵢ (L.sᴸ (⟦ · A ·  ⟧⁺) ([ f ]))
  [ O.⇐ₑ            f g ] = L.⇒ₑ ([ f ]) ([ g ])
  [ O.raa           f   ] = L.raa ([ f ])
  [ O.⇒ₑᵏ {Δ = Δ}   α f ] with map-lookup {f = ⟦_⟧ᵀ} Δ α
  [ O.⇒ₑᵏ {Γ} {Δ}   α f ] | β , p = L.⇒ₑᵏ β [f]
    where
      [f] : L.λC⁻ ⟦ Γ ⟧⁺ LJ.⊢[ ⟦ Δ ⟧⁻ ‼ β ] ⟦ Δ ⟧⁻
      [f] = subst (λ A → L.λC⁻ ⟦_⟧⁺ Γ LJ.⊢[ A ] ⟦ Δ ⟧⁻) p ([ f ])
  [ O.-ᵢ            f g ] = L.-ᵢ ([ f ]) ([ g ])
  [ O.-ₑ            f g ] = L.-ₑ ([ f ]) ([ g ])
  [ O.⊗ᵢ            f g ] = L.⊗ᵢ ([ f ]) ([ g ])
  [ O.⊗ₑ  {Γ₁} Γ₂ {A} {B} {C} f g ] = go {Γ₁} Γ₂ {A} {B} {C} ([ f ]) ([ g ])
    where
      open Monoid (Data.List.monoid Type) using (assoc; identity)

      go  : ∀ {Γ₁ : Conjunction} (Γ₂ : Context) {A B C Δ}
          → L.λC⁻ (⟦ Γ₁ ⟧⁺)                     LJ.⊢[ ⟦ A ⊗ B ⟧ᵀ ] (⟦ Δ ⟧⁻)
          → L.λC⁻ (⟦ (Γ₂ ⟨ · A · ⊗ · B · ⟩) ⟧⁺) LJ.⊢[ ⟦ C     ⟧ᵀ ] (⟦ Δ ⟧⁻)
          → L.λC⁻ (⟦ (Γ₂ ⟨ Γ₁ ⟩) ⟧⁺)            LJ.⊢[ ⟦ C     ⟧ᵀ ] (⟦ Δ ⟧⁻)
      go {Γ₁} Γ₂ {A} {B} {C} {Δ} f g
        = subst (λ Γ → L.λC⁻ Γ LJ.⊢[ ⟦ C ⟧ᵀ ] (⟦ Δ ⟧⁻)) (proj₂ identity (⟦ (Γ₂ ⟨ Γ₁ ⟩) ⟧⁺))
        $ go′ Γ₂ {∅} {A} {B} {C} f
        $ subst (λ Γ → L.λC⁻ Γ LJ.⊢[ ⟦ C ⟧ᵀ ] (⟦ Δ ⟧⁻)) (sym (proj₂ identity (⟦ (Γ₂ ⟨ · A · ⊗ · B · ⟩) ⟧⁺)))
        $ g
        where
        go′ : ∀ {Γ₁ : Conjunction} (Γ₂ : Context) {Γ₃ : List Type} {A B C Δ}
           → L.λC⁻ (⟦ Γ₁ ⟧⁺)                           LJ.⊢[ ⟦ A ⊗ B ⟧ᵀ ] (⟦ Δ ⟧⁻)
           → L.λC⁻ (⟦ (Γ₂ ⟨ · A · ⊗ · B · ⟩) ⟧⁺) ++ Γ₃ LJ.⊢[ ⟦ C     ⟧ᵀ ] (⟦ Δ ⟧⁻)
           → L.λC⁻ (⟦ (Γ₂ ⟨ Γ₁ ⟩) ⟧⁺)            ++ Γ₃ LJ.⊢[ ⟦ C     ⟧ᵀ ] (⟦ Δ ⟧⁻)
        go′ [] f g = L.⊗ₑ f g
        go′ {Γ₁} (Γ₂ ⊗> Γ₃) {Γ₄} {A} {B} {C} {Δ} f g
                rewrite assoc  (⟦ Γ₂ ⟧⁺) (⟦ Γ₃ ⟨       Γ₁      ⟩ ⟧⁺) Γ₄
                      | assoc  (⟦ Γ₂ ⟧⁺) (⟦ Γ₃ ⟨ · A · ⊗ · B · ⟩ ⟧⁺) Γ₄
                      = L.eᴸ ∅ (⟦ Γ₂ ⟧⁺) (⟦ Γ₃ ⟨ Γ₁ ⟩ ⟧⁺) Γ₄
                      $ go′ {Γ₁} Γ₃ {⟦ Γ₂ ⟧⁺ ++ Γ₄} {A} {B} {C} {Δ} f
                      $ L.eᴸ ∅ (⟦ Γ₃ ⟨ · A · ⊗ · B · ⟩ ⟧⁺) (⟦ Γ₂ ⟧⁺) Γ₄
                      $ g             
        go′ {Γ₁} (Γ₂ <⊗ Γ₃) {Γ₄} {A} {B} {C} {Δ} f g
                rewrite assoc (⟦ Γ₂ ⟨       Γ₁      ⟩ ⟧⁺) (⟦ Γ₃ ⟧⁺) Γ₄
                      | assoc (⟦ Γ₂ ⟨ · A · ⊗ · B · ⟩ ⟧⁺) (⟦ Γ₃ ⟧⁺) Γ₄
                      = go′ {Γ₁} Γ₂ {⟦ Γ₃ ⟧⁺ ++ Γ₄} {A} {B} {C} {Δ} f g
