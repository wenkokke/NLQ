...
module Logic.Classical.Ordered.LambekGrishin.FocPol.ProofSearch
...
open import Logic.Classical.Ordered.LambekGrishin.Type                (Polarity × Atom) as T
open import Logic.Classical.Ordered.LambekGrishin.Structure.Polarised (Polarity × Atom)
open import Logic.Classical.Ordered.LambekGrishin.Judgement           (Polarity × Atom) as J
open import Logic.Classical.Ordered.LambekGrishin.FocPol.Base         Atom


_≟-PolarisedAtom_ : (A B : Polarity × Atom) → Dec (A ≡ B)
(p , A) ≟-PolarisedAtom (q , B) with p ≟-Polarity q
... | no  p≠q = no (p≠q ∘ cong proj₁)
... | yes p=q rewrite p=q with A ≟-Atom B
... | no  A≠B = no (A≠B ∘ cong proj₂)
... | yes A=B rewrite A=B = yes refl


open T.DecEq _≟-PolarisedAtom_ using (_≟-Type_)
open J.DecEq _≟-PolarisedAtom_ using (_≟-Judgement_)
