...
module Logic.Intuitionistic.Unrestricted.Lambda.Untyped.ToLaTeX where
...
instance
  toLaTeXTerm : ∀ {n} → Vec String n → Term n → String
  toLaTeXTerm xs f = proj₁ (go′ (V.map (λ x → "\\text{" ++ x ++ "}") xs) f names)
    where
      ...
      go′ : ∀ {n} → Vec String n → Term n → State (Stream String) String
