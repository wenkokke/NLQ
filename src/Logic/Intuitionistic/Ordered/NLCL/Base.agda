...
module Logic.Intuitionistic.Ordered.NLCL.Base {ℓ} (Univ : Set ℓ) (I L R : Univ) where
...
data NL_ : Judgement → Set ℓ where
  ...
  -- rules for (I , L , R)
  Iᵢ  : ∀ {A B}     → NL A ⊢ B → NL A ∘ el I ⊢ B
  Iₑ  : ∀ {A B}     → NL A ∘ el I ⊢ B → NL A ⊢ B
  Lᵢ  : ∀ {A B C D} → NL A ∙ (B ∘ C) ⊢ D → NL B ∘ ((el L ∙ A) ∙ C) ⊢ D
  Lₑ  : ∀ {A B C D} → NL B ∘ ((el L ∙ A) ∙ C) ⊢ D → NL A ∙ (B ∘ C) ⊢ D
  Rᵢ  : ∀ {A B C D} → NL (A ∘ B) ∙ C ⊢ D → NL A ∘ ((el R ∙ B) ∙ C) ⊢ D
  Rₑ  : ∀ {A B C D} → NL A ∘ ((el R ∙ B) ∙ C) ⊢ D → NL (A ∘ B) ∙ C ⊢ D
...
Iᵢ′ : ∀ (Σ : Context +) {A B} → NL Σ [ A ] ⊢ B → NL Σ [ A ∘ el I ] ⊢ B
Iᵢ′ [] f = Iᵢ f
Iᵢ′ (_ ∙> Σ) f = r⇒∙ (Iᵢ′ Σ (r∙⇒ f))
Iᵢ′ (_ ∘> Σ) f = r⇛∘ (Iᵢ′ Σ (r∘⇛ f))
Iᵢ′ (Σ <∙ _) f = r⇐∙ (Iᵢ′ Σ (r∙⇐ f))
...
Iₑ′ : ∀ (Σ : Context +) {A B} → NL Σ [ A ] ∘ el I ⊢ B → NL Σ [ A ] ⊢ B
Iₑ′ [] f = Iₑ f
Iₑ′ (_ ∙> Σ) f = Iₑ f
Iₑ′ (_ ∘> Σ) f = Iₑ f
Iₑ′ (Σ <∙ _) f = Iₑ f
...
Lᵢ′ : ∀ (Σ : Context +) {A B C D} → NL Σ [ A ∙ (B ∘ C) ] ⊢ D → NL Σ [ B ∘ ((el L ∙ A) ∙ C) ] ⊢ D
Lᵢ′ [] f = Lᵢ f
Lᵢ′ (_ ∙> Σ) f = r⇒∙ (Lᵢ′ Σ (r∙⇒ f))
Lᵢ′ (_ ∘> Σ) f = r⇛∘ (Lᵢ′ Σ (r∘⇛ f))
Lᵢ′ (Σ <∙ _) f = r⇐∙ (Lᵢ′ Σ (r∙⇐ f))
...
Lₑ′ : ∀ (Σ : Context +) {A B C D} → NL Σ [ B ∘ ((el L ∙ A) ∙ C) ] ⊢ D → NL Σ [ A ∙ (B ∘ C) ] ⊢ D
Lₑ′ [] f = Lₑ f
Lₑ′ (_ ∙> Σ) f = r⇒∙ (Lₑ′ Σ (r∙⇒ f))
Lₑ′ (_ ∘> Σ) f = r⇛∘ (Lₑ′ Σ (r∘⇛ f))
Lₑ′ (Σ <∙ _) f = r⇐∙ (Lₑ′ Σ (r∙⇐ f))
...
Rᵢ′ : ∀ (Σ : Context +) {A B C D} → NL Σ [ (A ∘ B) ∙ C ] ⊢ D → NL Σ [ A ∘ ((el R ∙ B) ∙ C) ] ⊢ D
Rᵢ′ [] f = Rᵢ f
Rᵢ′ (_ ∙> Σ) f = r⇒∙ (Rᵢ′ Σ (r∙⇒ f))
Rᵢ′ (_ ∘> Σ) f = r⇛∘ (Rᵢ′ Σ (r∘⇛ f))
Rᵢ′ (Σ <∙ _) f = r⇐∙ (Rᵢ′ Σ (r∙⇐ f))
...
Rₑ′ : ∀ (Σ : Context +) {A B C D} → NL Σ [ A ∘ ((el R ∙ B) ∙ C) ] ⊢ D → NL Σ [ (A ∘ B) ∙ C ] ⊢ D
Rₑ′ [] f = Rₑ f
Rₑ′ (_ ∙> Σ) f = r⇒∙ (Rₑ′ Σ (r∙⇒ f))
Rₑ′ (_ ∘> Σ) f = r⇛∘ (Rₑ′ Σ (r∘⇛ f))
Rₑ′ (Σ <∙ _) f = r⇐∙ (Rₑ′ Σ (r∙⇐ f))
