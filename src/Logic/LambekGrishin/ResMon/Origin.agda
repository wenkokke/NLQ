...
module el where
  ...
  mutual
    ...
    origin? ((A ⇛> B) <⊢ ._) (res-⊗⇒ f)     = go ((_ ⊗> (A ⇛> B)) <⊢ _) f  (res-⊗⇒ [])
    origin? ((A ⇛> B) <⊢ ._) (res-⊗⇐ f)     = go (((A ⇛> B) <⊗ _) <⊢ _) f  (res-⊗⇐ [])
    origin? ((A ⇛> B) <⊢ ._) (mon-⇛  f₁ f₂) = go (B <⊢ _)               f₁ (mon-⇛ᴸ [] f₂)
    origin? ((A ⇛> B) <⊢ ._) (res-⇛⊕ f)     = go ((_ ⇛> (A ⇛> B)) <⊢ _) f  (res-⇛⊕ [])
    origin? ((A ⇛> B) <⊢ ._) (res-⊕⇛ f)     = go (B <⊢ _)               f  (res-⊕⇛ [])
    origin? ((A ⇛> B) <⊢ ._) (res-⇚⊕ f)     = go (((A ⇛> B) <⇚ _) <⊢ _) f  (res-⇚⊕ [])
    origin? ((A ⇛> B) <⊢ ._) (grish₁ f)     = go ((B <⊗ _) <⊢ _)        f  (grish₁ [])
    origin? ((A ⇛> B) <⊢ ._) (grish₂ f)     = go ((_ ⊗> B) <⊢ _)        f  (grish₂ [])
    origin? ((A ⇚> B) <⊢ ._) (res-⊗⇒ f)     = go ((_ ⊗> (A ⇚> B)) <⊢ _) f  (res-⊗⇒ [])
    origin? ((A ⇚> B) <⊢ ._) (res-⊗⇐ f)     = go (((A ⇚> B) <⊗ _) <⊢ _) f  (res-⊗⇐ [])
    origin? ((A ⇚> B) <⊢ ._) (mon-⇚  f₁ f₂) = go (_ ⊢> B)               f₂ (mon-⇚ᴿ f₁ [])
    origin? ((A ⇚> B) <⊢ ._) (res-⇛⊕ f)     = go ((_ ⇛> (A ⇚> B)) <⊢ _) f  (res-⇛⊕ [])
    origin? ((A ⇚> B) <⊢ ._) (res-⊕⇚ f)     = go (_ ⊢> (_ ⊕> B))        f  (res-⊕⇚ [])
    origin? ((A ⇚> B) <⊢ ._) (res-⇚⊕ f)     = go (((A ⇚> B) <⇚ _) <⊢ _) f  (res-⇚⊕ [])
    origin? ((A ⇚> B) <⊢ ._) (grish₃ f)     = go (_ ⊢> (_ ⊕> B))        f  (grish₃ [])
    origin? ((A ⇚> B) <⊢ ._) (grish₄ f)     = go (_ ⊢> (_ ⊕> B))        f  (grish₄ [])
    ...
    origin? ((A <⇛ B) <⊢ ._) (res-⊗⇒ f)     = go ((_ ⊗> (A <⇛ B)) <⊢ _) f  (res-⊗⇒ [])
    origin? ((A <⇛ B) <⊢ ._) (res-⊗⇐ f)     = go (((A <⇛ B) <⊗ _) <⊢ _) f  (res-⊗⇐ [])
    origin? ((A <⇛ B) <⊢ ._) (mon-⇛  f₁ f₂) = go (_ ⊢> A)               f₂ (mon-⇛ᴿ f₁ [])
    origin? ((A <⇛ B) <⊢ ._) (res-⇛⊕ f)     = go ((_ ⇛> (A <⇛ B)) <⊢ _) f  (res-⇛⊕ [])
    origin? ((A <⇛ B) <⊢ ._) (res-⊕⇛ f)     = go (_ ⊢> (A <⊕ _))        f  (res-⊕⇛ [])
    origin? ((A <⇛ B) <⊢ ._) (res-⇚⊕ f)     = go (((A <⇛ B) <⇚ _) <⊢ _) f  (res-⇚⊕ [])
    origin? ((A <⇛ B) <⊢ ._) (grish₁ f)     = go (_ ⊢> (A <⊕ _))        f  (grish₁ [])
    origin? ((A <⇛ B) <⊢ ._) (grish₂ f)     = go (_ ⊢> (A <⊕ _))        f  (grish₂ [])
    origin? ((A <⇚ B) <⊢ ._) (res-⊗⇒ f)     = go ((_ ⊗> (A <⇚ B)) <⊢ _) f  (res-⊗⇒ [])
    origin? ((A <⇚ B) <⊢ ._) (res-⊗⇐ f)     = go (((A <⇚ B) <⊗ _) <⊢ _) f  (res-⊗⇐ [])
    origin? ((A <⇚ B) <⊢ ._) (mon-⇚  f₁ f₂) = go (A <⊢ _)               f₁ (mon-⇚ᴸ [] f₂)
    ...
    origin? ((A <⇚ B) <⊢ ._) (res-⇚⊕ f)     = go (((A <⇚ B) <⇚ _) <⊢ _) f  (res-⇚⊕ [])
    origin? ((A <⇚ B) <⊢ ._) (grish₃ f)     = go ((_ ⊗> A) <⊢ _)        f  (grish₃ [])
    origin? ((A <⇚ B) <⊢ ._) (grish₄ f)     = go ((A <⊗ _) <⊢ _)        f  (grish₄ [])
    ...
    origin? (._ ⊢> (A ⇒> B)) (res-⊕⇛ f)     = go (_ ⊢> (_ ⊕> (A ⇒> B))) f  (res-⊕⇛ [])
    origin? (._ ⊢> (A ⇒> B)) (res-⊕⇚ f)     = go (_ ⊢> ((A ⇒> B) <⊕ _)) f  (res-⊕⇚ [])
    origin? (._ ⊢> (A ⇒> B)) (grish₂ f)     = go (_ ⊢> (_ ⊕> B))        f  (grish₂ [])
    origin? (._ ⊢> (A ⇒> B)) (grish₃ f)     = go (_ ⊢> (B <⊕ _))        f  (grish₃ [])
    ...
    origin? (._ ⊢> (A <⇒ B)) (res-⊕⇛ f)     = go (_ ⊢> (_ ⊕> (A <⇒ B))) f  (res-⊕⇛ [])
    origin? (._ ⊢> (A <⇒ B)) (res-⊕⇚ f)     = go (_ ⊢> ((A <⇒ B) <⊕ _)) f  (res-⊕⇚ [])
    origin? (._ ⊢> (A <⇒ B)) (grish₂ f)     = go ((A <⊗ _) <⊢ _)        f  (grish₂ [])
    origin? (._ ⊢> (A <⇒ B)) (grish₃ f)     = go ((A <⊗ _) <⊢ _)        f  (grish₃ [])
    origin? (._ ⊢> (A <⇐ B)) (mon-⇐  f₁ f₂) = go (_ ⊢> A)               f₁ (mon-⇐ᴸ [] f₂)
    origin? (._ ⊢> (A <⇐ B)) (res-⇒⊗ f)     = go (_ ⊢> (_ ⇒> (A <⇐ B))) f  (res-⇒⊗ [])
    origin? (._ ⊢> (A <⇐ B)) (res-⇐⊗ f)     = go (_ ⊢> ((A <⇐ B) <⇐ _)) f  (res-⇐⊗ [])
    ...
    origin? (._ ⊢> (A <⇐ B)) (res-⊕⇛ f)     = go (_ ⊢> (_ ⊕> (A <⇐ B))) f  (res-⊕⇛ [])
    origin? (._ ⊢> (A <⇐ B)) (res-⊕⇚ f)     = go (_ ⊢> ((A <⇐ B) <⊕ _)) f  (res-⊕⇚ [])
    origin? (._ ⊢> (A <⇐ B)) (grish₁ f)     = go (_ ⊢> (_ ⊕> A))        f  (grish₁ [])
    origin? (._ ⊢> (A <⇐ B)) (grish₄ f)     = go (_ ⊢> (A <⊕ _))        f  (grish₄ [])
